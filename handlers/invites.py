...
async def cmd_invites(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type not in ("group", "supergroup"):
        ...
        return
    count = await store.get_invites(chat.id, user.id)
    try:
        ...
        return
    await store.map_link_to_inviter(chat.id, link.invite_link, user.id)
    await store.set_inviter_name(chat.id, user.id, user.first_name)
...
async def on_join_request(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await store.inc_invite(chat.id, inviter_id, "")
    try:
        await req.approve()
...
async def cmd_invitetop(update: Update, context: ContextTypes.DEFAULT_TYPE):
    chat = update.effective_chat
    top = await store.top_inviters(chat.id, limit=10)
    if not top:
