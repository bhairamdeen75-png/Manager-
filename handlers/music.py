...
async def _get(chat_id):
    return await db.get_music_queue(chat_id)


async def _save(chat_id, tracks, current=None, current_msg_id=None):
    await db.save_music_queue(chat_id, tracks, current, current_msg_id)
...
async def cmd_play(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if current and doc.get("current_msg_id"):
        ...
        await _save(chat_id, tracks, current=track, current_msg_id=msg_id)
    ...
    if action == "stop":
        ...
        if doc.get("current_msg_id"):
            try:
                ...
                pass
        await db.clear_music_queue(chat_id)
        ...
        await _save(chat_id, tracks, current=tracks[0], current_msg_id=doc.get("current_msg_id"))
        await _skip(context, chat_id, "next", editor=query, user=query.from_user.first_name)
    else:
        await query.answer()
