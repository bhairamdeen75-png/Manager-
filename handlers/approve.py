...
async def cmd_approve(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.reply_to_message and msg.reply_to_message.from_user:
        ...
        try:
            ...
            target = None
            await store.add_approved(update.effective_chat.id, uid, str(uid))
    if not target:
        await msg.reply_text("Kisi user pe reply karke /approve, ya /approve <user_id>")
        return
    await store.add_approved(update.effective_chat.id, target.id, target.first_name)
...
async def cmd_unapprove(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = update.effective_message
    if msg.reply_to_message and msg.reply_to_message.from_user:
        await store.remove_approved(update.effective_chat.id, msg.reply_to_message.from_user.id)
        await msg.reply_text("🧹 Unapproved.")
    elif context.args and context.args[0].lstrip("-").isdigit():
        await store.remove_approved(update.effective_chat.id, int(context.args[0]))
        await msg.reply_text("🧹 Unapproved.")
    else:
...
async def cmd_approved(update: Update, context: ContextTypes.DEFAULT_TYPE):
    users = await store.get_approved(update.effective_chat.id)
    if not users:
