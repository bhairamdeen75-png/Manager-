...
async def _get(chat_id, key, default):
    doc = await db.settings_col.find_one({"chat_id": chat_id})
    return doc.get(key, default) if doc else default

async def _set(chat_id, key, value):
    await db.settings_col.update_one({"chat_id": chat_id}, {"$set": {key: value}}, upsert=True)
...
async def cmd_allowdomain(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    wl = await _get(update.effective_chat.id, "url_whitelist", [])
    if domain in wl:
        await update.message.reply_text(f"😅 {domain} pehle se allowed hai bhai.")
        return
    wl.append(domain)
    await _set(update.effective_chat.id, "url_whitelist", wl)
...
async def cmd_securitystatus(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    wl = await _get(chat_id, "url_whitelist", [])
    quar = await _get(chat_id, "quarantine", False)
    night = await db.get_night_mode(chat_id)
    raid = await db.get_raid_protection(chat_id)
    text = (
...
async def cmd_quarantine(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or context.args[0].lower() not in ("on", "off"):
        cur = "on" if await _get(update.effective_chat.id, "quarantine", False) else "off"
    ...
    await _set(update.effective_chat.id, "quarantine", enabled)
    if enabled:
