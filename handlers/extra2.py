...
# ---------------- /guess — group guessing game 🎮 (Turso-backed) ----------------
...
async def cmd_guess(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        game = await db.get_guess_game(chat_id)
        if game:
            await update.message.reply_text(
        ...
        await db.set_guess_game(chat_id, number)
        await update.message.reply_text(
    ...
    secret = game["number"]
    await db.add_guess_try(chat_id)
    ...
    if secret > guess:
        ...
        tries = game["tries"] + 1
        await db.delete_guess_game(chat_id)
        if tries <= 5:
