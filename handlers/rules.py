...
async def cmd_rules(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = await db.get_rules(update.effective_chat.id) or DEFAULT_RULES
...
async def cmd_rulesgate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await db.set_rules_gate(update.effective_chat.id, enabled)
...
async def on_new_member_rules_gate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat.type not in ("group", "supergroup"):
        return
    if not await db.get_rules_gate(chat.id):
        return

    rules_text = await db.get_rules(chat.id) or DEFAULT_RULES
