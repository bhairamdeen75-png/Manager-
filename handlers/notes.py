...
async def cmd_save(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not content:
        ...
        return
    await db.add_note(update.effective_chat.id, name, content)
...
async def cmd_clear(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args:
        ...
        return
    await db.remove_note(update.effective_chat.id, context.args[0])
    await update.message.reply_text(f"🗑️ Note '#{context.args[0].lower()}' hata diya.")


async def cmd_notes(update: Update, context: ContextTypes.DEFAULT_TYPE):
    names = await db.list_notes(update.effective_chat.id)
    if not names:
...
async def check_note_trigger(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for match in HASHTAG_REGEX.finditer(msg.text):
        content = await db.get_note(chat.id, match.group(1))
        if content:
