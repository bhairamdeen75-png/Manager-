...
async def cmd_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    s = await db.get_stats(chat_id)
    filters_count = len(await db.get_filters(chat_id))
    notes_count = len(await db.list_notes(chat_id))
    leaderboard = await db.get_leaderboard(chat_id, limit=1)
