...
async def cmd_setwelcome(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await db.set_welcome(update.effective_chat.id, text)
...
async def cmd_setleave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not await is_admin(update, context):
        ...
        return
    if not context.args:
        current = await db.get_leave_message(update.effective_chat.id)
        await update.message.reply_text(
    ...
    if text.lower() in ("off", "disable", "none"):
        await db.set_leave_message(update.effective_chat.id, "")
        await update.message.reply_text("🧹 Leave message OFF kar diya.")
        return
    await db.set_leave_message(update.effective_chat.id, text)
    preview = format_welcome_leave(text, update.effective_user, update.effective_chat)
    await update.message.reply_text(
