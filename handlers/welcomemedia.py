...
async def cmd_setwmedia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not r:
        ...
        return
    if r.photo:
        await store.set_welcome_media(update.effective_chat.id, r.photo[-1].file_id, "photo")
    elif r.animation:
        await store.set_welcome_media(update.effective_chat.id, r.animation.file_id, "animation")
    elif r.video:
        await store.set_welcome_media(update.effective_chat.id, r.video.file_id, "video")
    else:
...
async def cmd_setwbtn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not buttons:
        ...
        return
    await store.set_welcome_buttons(update.effective_chat.id, buttons)
...
async def cmd_clearwmedia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await is_admin(update, context):
        await store.clear_welcome_media(update.effective_chat.id)
...
async def cmd_clearwbtn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if await is_admin(update, context):
        await store.clear_welcome_buttons(update.effective_chat.id)
...
async def on_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    media = await store.get_welcome_media(chat.id)
    buttons = await store.get_welcome_buttons(chat.id)
