...
async def cmd_setlinkblock(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await db.set_link_block(update.effective_chat.id, enabled)
...
async def cmd_restrictmedia(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await is_admin(update, context):
        ...
        return
    if not context.args:
        current = await db.get_media_restrict(update.effective_chat.id)
    ...
    if args == ["none"]:
        await db.set_media_restrict(update.effective_chat.id, "")
        await update.message.reply_text("✅ Media restrictions clear kar di.")
        return
    if args == ["all"]:
        await db.set_media_restrict(update.effective_chat.id, ",".join(MEDIA_TYPES))
    ...
    if not valid:
        ...
        return
    await db.set_media_restrict(update.effective_chat.id, ",".join(valid))
...
async def check_links(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if not msg or not msg.text or chat.type not in ("group", "supergroup"):
        return False
    if not await db.get_link_block(chat.id):
        return False
...
async def check_media(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if not msg or chat.type not in ("group", "supergroup"):
        return False
    restricted = await db.get_media_restrict(chat.id)
    if not restricted:
        return False
    if await is_admin(update, context):
        return False
