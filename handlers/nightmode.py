...
async def _night_start(context: ContextTypes.DEFAULT_TYPE):
    ...
    if not await db.get_night_mode(chat_id):
        return
    try:
...
async def schedule_all_night_modes(application):
    """Called once on bot startup to re-arm night mode jobs for every group that has it enabled."""
    for row in await db.get_all_night_mode_chats():
        chat_id = row["chat_id"]
...
async def cmd_nightmode(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or context.args[0].lower() not in ("on", "off"):
        start, end = await db.get_night_time(update.effective_chat.id)
    ...
    await db.set_night_mode(chat_id, enabled)

    if enabled:
        start, end = await db.get_night_time(chat_id)
...
async def cmd_setnighttime(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_id = update.effective_chat.id
    await db.set_night_time(chat_id, start, end)
    if await db.get_night_mode(chat_id):
        schedule_night_jobs(context, chat_id, start, end)
