...
async def award_xp(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await db.add_xp(chat.id, user.id, amount, cooldown_seconds=XP_COOLDOWN_SECONDS)


async def cmd_rank(update: Update, context: ContextTypes.DEFAULT_TYPE):
    from handlers.utils import get_target_user
    target = get_target_user(update) or update.effective_user
    xp = await db.get_xp(update.effective_chat.id, target.id)
...
async def cmd_leaderboard(update: Update, context: ContextTypes.DEFAULT_TYPE):
    rows = await db.get_leaderboard(update.effective_chat.id, limit=10)
    if not rows:
...
async def post_hourly_leaderboards(context: ContextTypes.DEFAULT_TYPE):
    ...
    chat_ids = await db.get_xp_chats()
    for chat_id in chat_ids:
        rows = await db.get_leaderboard(chat_id, limit=10)
        if not rows:
            continue
