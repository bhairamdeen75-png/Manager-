...
async def cmd_autodeletejoinleave(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not context.args or context.args[0].lower() not in ("on", "off"):
        current = await db.get_autodelete_joinleave(update.effective_chat.id)
        delay = await db.get_autodelete_delay(update.effective_chat.id)
    ...
    if not enabled:
        await db.set_autodelete_joinleave(update.effective_chat.id, False)
    ...
    await db.set_autodelete_joinleave(update.effective_chat.id, True)
    await db.set_autodelete_delay(update.effective_chat.id, delay)
    if delay == 0:
        await update.message.reply_text(
...
async def on_join_leave_service_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.left_chat_member:
        ...
        await db.remove_seen_user(chat.id, leaver.id)

        # Custom leave message (bots skip karo)
        leave_text = await db.get_leave_message(chat_id=chat.id)
    ...
    if not await db.get_autodelete_joinleave(chat.id):
        return

    delay = await db.get_autodelete_delay(chat.id)
    if delay == 0:
        try:
            await msg.delete()
        except Exception:
