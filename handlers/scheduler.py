"""/schedule 2h Movie night! — bot khud post karega. Restart-proof (Turso persist)."""
...
async def run_scheduled(context: ContextTypes.DEFAULT_TYPE):
    ...
    await store.del_schedule(data["chat_id"], data["sched_id"])
...
async def cmd_schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    sched_id = str(await store.add_schedule(update.effective_chat.id, run_at, text,
...
async def cmd_scheduled(update: Update, context: ContextTypes.DEFAULT_TYPE):
    scheds = await store.get_chat_schedules(update.effective_chat.id)
    if not scheds:
