...
async def _get_quarantine(chat_id) -> bool:
    doc = await db.settings_col.find_one({"chat_id": chat_id})
    return doc.get("quarantine", False) if doc else False


async def _set_quarantine(chat_id, value: bool):
    await db.settings_col.update_one({"chat_id": chat_id}, {"$set": {"quarantine": value}}, upsert=True)
...
async def _state(gid, getter) -> bool:
    try:
        return bool(await getter(gid))
    except Exception:
        return False
...
async def show_layer(update, context, chat_id: int, layer_key: str):
    ...
    for key, (label, emoji, getter, _setter) in layer["toggles"].items():
        state = await _state(chat_id, getter)
...
async def do_toggle(update, context, chat_id: int, layer_key: str, key: str):
    ...
    new_val = not await _state(chat_id, getter)
    try:
    ...
    for k2, (label2, emoji2, getter2, _s2) in layer["toggles"].items():
        st = await _state(chat_id, getter2)
